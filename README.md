# CSS Flexbox Photo Gallery

This project creates a **responsive image gallery** using the **CSS Flexbox layout**. It is a common tutorial from resources like **freeCodeCamp**, designed to demonstrate modern CSS techniques for building dynamic user interfaces.

---

##  Features

This project includes the following features (using GFM task lists):

- [x] **Responsive Design**  
  The gallery adapts to different screen sizes and device orientations.

- [x] **Flexbox Layout**  
  Utilizes `display: flex`, `flex-wrap`, and `justify-content` for image arrangement.

- [x] **Image Handling**  
  Uses `object-fit: cover` to ensure images fill their containers uniformly without distortion.

- [x] **Placeholder Content**  
  Uses an assortment of fun cat images as visual placeholders.

---

---

##  Technologies Used

| Technology | Purpose |
|-----------|---------|
| **HTML5** | Provides the basic structure of the webpage |
| **CSS3**  | Handles styling and the core Flexbox layout logic |

---
